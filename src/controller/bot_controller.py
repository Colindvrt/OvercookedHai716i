...
class AIBot:
    def __init__(self, player_index: int = 0, shared_knowledge: Optional[SharedKnowledge] = None):
        ...
        self._wait_retry_count = 0  # 🆕 Compteur pour éviter les boucles infinies
        self._max_wait_retries = 10  # 🆕 Maximum de tentatives avant abandon
    ...
    def _check_task_completion(self, m: GameModel, task: Task) -> bool:
        ...
        # ✅ FIX #4 : Vérifier que la commande existe encore
        elif task.task_type == TaskType.DELIVER:
            # Vérifier si la commande existe toujours
            order_exists = any(o.id == task.order_id for o in m.orders)
            if not order_exists:
                return True  # Commande disparue -> considérer comme terminée
            
            # Si la commande existe, vérifier si on a livré (plat disparu de nos mains après interaction)
            for order in m.orders:
                if order.id == task.order_id:
                    return False  # Commande encore active -> Pas encore livré
            return True
    ...
    def update(self, m: GameModel):
            ...
            for order_id in list(self.shared_knowledge.task_lists.keys()):
                if not any(o.id == order_id for o in m.orders):
                    ...
                    if self.current_task and self.current_task.order_id == order_id:
                        ...
                        self.queue.clear()
                        self._wait_retry_count = 0  # 🆕 Reset du compteur
                        self._clear_hands(m)
        ...
        if not self.current_task:
            ...
            if next_task:
                ...
                self._wait_retry_count = 0  # 🆕 Reset du compteur pour nouvelle tâche
        
        if self.current_task:
            # 🆕 FIX #4 : Vérifier si la commande existe toujours avant de continuer
            order_exists = any(o.id == self.current_task.order_id for o in m.orders)
            if not order_exists and self.current_task.task_type != TaskType.DELIVER:
                # La commande a expiré pendant l'exécution (sauf si c'est une tâche DELIVER en cours)
                self.shared_knowledge.complete_task(self.current_task, self.player_index)
                self.current_task = None
                self.queue.clear()
                self._wait_retry_count = 0
                self._clear_hands(m)
                return
        ...
        if task.task_type == TaskType.TAKE:
            ...
            if board:
                if self.shared_knowledge.reserve_station(board, self.player_index):
                    ...
                    self._push(Step.INTERACT, board)
                    self._wait_retry_count = 0  # 🆕 Reset si succès
                else:
                    # 🆕 FIX #2 : Gestion du timeout sur les réservations
                    self._wait_retry_count += 1
                    if self._wait_retry_count >= self._max_wait_retries:
                        # Abandon de la tâche après trop d'attentes
                        self.shared_knowledge.complete_task(self.current_task, self.player_index)
                        self.current_task = None
                        self._wait_retry_count = 0
                    else:
                ...
                # 🆕 FIX #2 : Même logique si aucune planche libre
                self._wait_retry_count += 1
                if self._wait_retry_count >= self._max_wait_retries:
                    self.shared_knowledge.complete_task(self.current_task, self.player_index)
                    self.current_task = None
            ...
            if task.item_type == ItemType.RAW_PATTY:
                ...
                if cooked_furnace:
                    # 🆕 FIX #3 : Ne pas retourner immédiatement après _clear_hands
                    if p.held_item:
                        self._clear_hands(m)
                        return  # On attend le prochain update pour récupérer la pizza
                    else:
            ...
            assembly = self._assembly(m)
            if assembly:
                # 🆕 Vérifier si on peut poser l'ingrédient (ordre de la recette)
                can_place = self._can_place_ingredient(m, assembly, task)
                
                if not can_place:
                    # Les ingrédients précédents ne sont pas encore posés, attendre
                    self._push(Step.WAIT, None, 0.3)
                    return
            ...
            # 🆕 FIX #4 : Vérifier que la commande existe toujours
            order_exists = any(o.id == task.order_id for o in m.orders)
            if not order_exists:
                # La commande a expiré, abandonner la tâche
                self.shared_knowledge.complete_task(self.current_task, self.player_index)
                self.current_task = None
                self._wait_retry_count = 0
                if p.held_item:
    ...
    def _clear_hands(self, m):
        # 🆕 FIX #3 : Gestion améliorée quand aucune planche n'est libre
        s = self._free_board(m)
        if s:
            self._push(Step.GO_TO, s)
            self._push(Step.INTERACT, s)
        else:
            # Si aucune planche libre, attendre un peu avant de réessayer
            # On ne fait rien ici, le prochain update tentera à nouveau
            self._push(Step.WAIT, None, 0.3)

    def _can_place_ingredient(self, m: GameModel, assembly: Station, task: Task) -> bool:
        """
        Vérifie si l'ingrédient peut être placé selon l'ordre de la recette.
        Retourne True si tous les ingrédients précédents sont déjà sur l'assembly.
        """
        # ⚠️ ATTENTION : Pour BURGER et SALAD, l'ordre n'a PAS d'importance (Set dans _check_recipe_completion)
        # Donc on ne vérifie l'ordre QUE pour la PIZZA
        
        if not task.order_id:
            return True  # Pas de vérification si pas d'order_id
        
        # Récupérer la commande associée
        order = None
        for o in m.orders:
            if o.id == task.order_id:
                order = o
                break
        
        if not order or not order.items_needed:
            return True  # Si la commande n'existe plus, on peut essayer de placer
        
        # Récupérer le type de plat demandé
        dish_type = order.items_needed[0] if order.items_needed else None
        
        # ✅ Pour BURGER et SALAD : l'ordre n'a pas d'importance (Set)
        if dish_type in [ItemType.BURGER, ItemType.SALAD]:
            return True
        
        # ✅ Pour PIZZA : ordre strict = BREAD, TOMATO, CHEESE
        if dish_type == ItemType.PIZZA:
            pizza_order = [ItemType.BREAD, ItemType.TOMATO, ItemType.CHEESE]
            
            # Trouver l'index de notre ingrédient dans la recette
            our_index = -1
            for i, ingredient in enumerate(pizza_order):
                if ingredient == task.item_type:
                    our_index = i
                    break
            
            if our_index == -1:
                return True  # Ingrédient pas dans la recette pizza
            
            if our_index == 0:
                return True  # Premier ingrédient (BREAD), on peut toujours le placer
            
            # Vérifier que TOUS les ingrédients précédents sont déjà sur l'assembly
            for i in range(our_index):
                required_ingredient = pizza_order[i]
                ingredient_found = False
                
                # Chercher dans assembly.contents
                for item in assembly.contents:
                    if item.item_type == required_ingredient:
                        # Pour la pizza, vérifier si TOMATO doit être coupé
                        if required_ingredient == ItemType.TOMATO and not item.chopped:
                            continue  # Tomate pas encore coupée
                        ingredient_found = True
                        break
                
                # Chercher dans assembly.item (cas où il n'y a qu'un seul item)
                if not ingredient_found and assembly.item:
                    if assembly.item.item_type == required_ingredient:
                        if required_ingredient == ItemType.TOMATO:
                            if assembly.item.chopped:
                                ingredient_found = True
                        else:
                            ingredient_found = True
                
                if not ingredient_found:
                    return False  # Un ingrédient précédent manque
        
        return True  # Tous les ingrédients précédents sont présents ou pas de contrainte d'ordre